...
import * as data01 from './../testdata/data.login.json';
...
test('register user with existing email', async({page, baseURL})=>{
    url = `${baseURL}`;
    homePage = new HomePage(page);
    registerPage = new RegisterPage(page);
    accountPage = new AccountPage(page);
    ...
    await registerPage.verifyNewUserSignupVisible();
    await registerPage.createNewAccount(data01.username, data01.email);
    await registerPage.verifyDisplayEmailExisted()
})
